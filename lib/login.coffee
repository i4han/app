...
login = ->
    ...
    Meteor.loginWithPassword usernameOrEmail, password, (err, result) -> 
...
module.exports.login =
    ...
    login: 
        ...
        events:
            ...
            'click .login-close': -> closeDropdown() ; $('#login-dropdown-list').removeClass 'open'
            'click #login-name-link, click #login-sign-in-link': (event) -> 
                event.stopPropagation()
                Session.set 'login.dropdownVisible', true
        helpers:
            template: -> 'dropdown_' + if !Meteor.user() then 'logged_out' else 
    ...
    dropdown_logged_in:
        jade: """
            li.dropdown#login-dropdown-list
                ...
                .dropdown-menu#dropdown-menu-insert
                    +Template.dynamic(template=template)
            """
        styl$: (Config) -> """
            #logged-in-dropdown
        ...
        rendered: ->
            insert = 'dropdown-menu-insert'
            template = if Session.get 'login.inMessageOnlyFlow' then 'login_messages' else
                if Session.get 'login.inChangePasswordFlow' then 'change_password' else 'dropdown_menu_logged_in'
            __.insertTemplate template, insert
            if Session.get('login.inMessageOnlyFlow') or Session.get('login.inChangePasswordFlow')
                $('#'+insert).wrapInner '<div id="#logged-in-dropdown"></div>'
