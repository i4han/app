...
keyFix = function(key) {
  ...
  switch (false) {
    case !((i = key.indexOf('%')) > 0):
      return key.slice(0, +(i - 1) + 1 || 9e9);
    case 0 !== i:
      return '';
    default:
      return key;
...
tideKey = function(key, prefix, seperator) {
  var r;
  (r = key.match(/%\{\s*([a-z0-9]+)\s*\}/)) && (key = key.replace(/%\{\s*[a-z0-9]+\s*\}/, prefix + '-' + r[1]));
  if ((/[ #+.-]+/.test(key)) || (/^h[1-6]$/.test(key)) || (key.indexOf('_') === -1) && (!/[0-9]+/.test(key))) {
    return key;
  }
  return key.split('_').map(function(a) {
    switch (false) {
      case '' !== a:
        return void 0;
      case !/[0-9]+/.test(a):
...
x.tideKey = function(obj, prefix, seperator) {
  if (!x.isObject(obj)) {
    return obj;
  }
  return x.keys(obj).reduce((function(o, k) {
    var ok;
    o[tideKey(k, prefix, seperator)] = (function() {
      switch (false) {
        case !x.isObject(ok = obj[k]):
          return x.tideKey(ok, prefix, seperator);
        default:
          return ok;
      }
    })();
...
tideEventKey = function(k, prefix) {
  var r;
...
x.tideEventKey = function(obj, prefix) {
  return x.keys(obj).reduce((function(o, k) {
    return x.addProperty(o, tideEventKey(k, prefix), obj[k]);
  }), {});
};
...
x.module = function(name) {
  var i;
  return (i = new x.Module(name))._instance(i);
};
