...
var NotPxDefaultProperties, validateRe,
...
x.addpx = function(obj) {
  return x.keys(obj).map(function(k) {
    var ok;
    return [k, x.isObject(ok = obj[k]) ? x.addpx(ok) : 0 === ok ? '0' : 'number' === typeof ok ? String(ok) + (__indexOf.call(NotPxDefaultProperties, k) >= 0 ? '' : 'px') : ok];
  }).reduce((function(o, v) {
    o[v[0]] = v[1];
    return o;
  }), {});
